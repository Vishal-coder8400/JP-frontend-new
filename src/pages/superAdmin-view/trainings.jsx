...
import {
  useGetAllTrainings,
  useDeleteTraining,
} from "../../hooks/superAdmin/useTraining";
...
import { Search, Eye, Trash2, Plus, Calendar, Users } from "lucide-react";
...
const Trainings = () => {
  ...
  const { data: trainingsData, isLoading } = useGetAllTrainings({
    page,
    limit: 10,
    search,
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md border border-gray-200">
        <div className="p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">
            Training Programs ({trainingsData?.data?.totalTrainings || 0})
          </h3>
          ...
          {isLoading ? (
            ...
            <Table>
              ...
              <TableBody>
                {trainingsData?.data?.trainings?.map((training) => (
                  <TableRow key={training.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        <img
                          src={training.instructor?.avatar || "/image.png"}
                          ...
                          <DialogContent>
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                ...
                                {training.title}"? This action cannot be undone.
                              </DialogDescription>
                            </DialogHeader>
                            <DialogFooter>
      ...
      <div className="flex justify-center space-x-2">
        ...
        <span className="px-4 py-2 text-sm text-gray-600">
          Page {page} of{" "}
          {Math.ceil((trainingsData?.data?.totalTrainings || 0) / 10)}
        </span>
        <Button
          variant="outline"
          onClick={() => setPage(page + 1)}
          disabled={
            page >= Math.ceil((trainingsData?.data?.totalTrainings || 0) / 10)
          }
        >
          Next
        </Button>
