...
const Profile = () => {
  ...
  const pdfFiles = Object.entries(pdfObject).reduce(
    (acc, [customKey, path]) => {
      acc[customKey] = getValue(user, path);
      return acc;
    },
    {}
  );
              ...
              <div className="flex items-center justify-between w-full gap-[12px]">
                ...
                <div className="max-w-[237px] w-full h-[125px] flex flex-col p-[16px] gap-[12px] rounded-[8px] border-[#DBE0E5] border-[1px]">
                  ...
                  <div className="text-[#61758A] text-base font-normal leading-tight line-clamp-2">
                    {`${user?.currentAddress?.address}, ${user?.currentAddress?.city}, ${user?.currentAddress?.state}`}
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="self-stretch inline-flex flex-wrap justify-between items-center gap-3">
                {Object.entries(pdfFiles).map(([key, value]) => {
                  ...
                  return (
                    <div
                      key={key}
                      className="relative overflow-hidden p-3 w-[180px] h-[208px] bg-stone-50 rounded-lg inline-flex flex-col gap-2"
                    >
                      ...
                      <div className="absolute bottom-0 left-0 w-full px-3 py-1 bg-stone-50 text-zinc-600 text-xs truncate">
                        {fileName}
                      </div>
                    </div>
                  );
