...
const RecruitersTab = ({ context = "database" }) => {
  ...
  return context === "approvals"
    ? data?.data?.approvals?.map((approval) => {
        const name = approval?.applicantDetails?.name || "-";
        const firstName = approval?.applicantDetails?.firstName || "";
        const lastName = approval?.applicantDetails?.lastName || "";
        const profileImage = approval?.applicantDetails?.profileImage;
        
        // If no profile image, create initials from name
        let displayImage = profileImage;
        if (!profileImage || profileImage === "N/A" || profileImage.trim() === "") {
          const getInitials = (firstName, lastName) => {
            const firstInitial = firstName ? firstName.charAt(0).toUpperCase() : "";
            const lastInitial = lastName ? lastName.charAt(0).toUpperCase() : "";
            return firstInitial + lastInitial;
          };
          displayImage = getInitials(firstName, lastName);
        }
        ...
        return {
          ...
          contact: approval?.data?.phone
            ? `${approval.data.phone?.countryCode || ""} ${
                approval.data.phone?.number || ""
              }`.trim()
            : "-",
